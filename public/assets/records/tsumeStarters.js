...
const boardDataSet = [
    {
        title: 'Problem #0',
        caption: '3 movestsume chackMate problem ',
...
boardDataSet.forEach((problem, index) => {
    select.options[select.options.length] = new Option(problem.title, index);
})


const setData = (index) => {
    const boardData = boardDataSet[index]
...
nextButton.addEventListener('click', e => {
    e.preventDefault();
    if (select.selectedIndex === boardDataSet.length - 1) select.selectedIndex = 0;
...
prevButton.addEventListener('click', e => {
    e.preventDefault();
    if (select.selectedIndex === 0) select.selectedIndex = boardDataSet.length - 1;
    else select.selectedIndex -= 1;
    setData(select.selectedIndex);
})
